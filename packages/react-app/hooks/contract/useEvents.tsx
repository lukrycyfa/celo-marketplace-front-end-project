...
// for buyers from this market place, that have utilizing useApprove hook to approve the purchase of a product
// import dependencies and utilities from react.
import { useState, useEffect } from "react";
// Import the `useContractEvent` hook to from wagmi to watch for events
...
export const useContractEvents = (eventName: String | any, address: String | any, confirm: boolean) => {
    ...
    // Sets the state for logs returned from the event
    ...
    // if args from the approved event matches the caller adddress, the marketplace address and `confirm` from the requesting component ('product') is `true`
    useEffect(() => {
        if (logs.length > 0) {
